...
/**
 * @brief Timer 0 interrupt handler with added keyboard led toggling
 *
 * if a second passes (counter % 60 == 0) it toggles the led passed in the parameter led
 *
 * @param j, led array counter, is incremented when the above condition is verified
 * @param led, led to be toggled on/off (0 scroll lock/ 1 num lock/ 2 caps lock)
 */
